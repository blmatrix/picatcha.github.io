...
function slideForward() {
   ...
   if ( $active.length == 0 ) $active = $('#showcase .showcase:first');
   
   var $next = $active.next();  
   var firstSlide = $('#showcase .showcase:first');
   var slideNum = '';
   
   // if at end of slide list, loop back
   if (!$next.attr('id')){
     ...
     slideNum = $next.attr('id').substring(8);
   }
   
   $active.fadeOut(1000).removeClass('active');     
   changeActiveIndicator(slideNum);
}
...
function slideBack() {
  ...
  var lastSlide = $('#showcase .showcase:last');
  var slideNum = '';
 
  // if at beginning of slide list, loop to end
  if (!$prev.attr('id')){
    lastSlide.fadeIn(1000).addClass('active');
    slideNum = lastSlide.attr('id').substring(8);
  }
  else {
    $prev.fadeIn(1000).addClass('active').css('z-index' , '11');
    slideNum = $prev.attr('id').substring(8);
  }
...
function jumpSlide(slideNum){
  
  var $active = $('#showcase div.active');
  var newSlide = $('#showcase' + slideNum);
  
  newSlide.fadeIn(1000).addClass('active');
  $active.fadeOut(1000).removeClass('active');

  changeActiveIndicator(slideNum);
}

function changeActiveIndicator(slideNum){
  
  $('#jumpSlides a.current').removeClass('current');
  $('#jumpSlide' + slideNum).addClass('current');
}

$(window).load(function() {
  
  $('#showcase .showcase') .not('.active') .css('display' , 'none');
    
  if ($('#showcase1').length) {
    
    var intervalId = setInterval(slideForward, 7000 );
  
    $('#showcase').click(function() {
      clearInterval(intervalId);
    });

    $('#showcaseNav .next').click(function() {
      clearInterval(intervalId);
      slideForward();
    });

    $('#showcaseNav .prev').click(function() {
      clearInterval(intervalId);
      slideBack();
    });
    
    $('#jumpSlides a').click(function() {
      clearInterval(intervalId);
    });
  }
});
