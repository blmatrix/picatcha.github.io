...
function renderMedianetAds()
{
    var t_medium = getUrlParameter('utm_medium');
    var t_source = getUrlParameter('utm_source');
    
    var excludeSources = ['e_search', 'bl', 'pdamv.bidsystem.com', 'kcmv.bidsystem.com', 'clickshieldfilter.com', 'mfp', 'avoplus.net'];

    var exclude_medianet_ads = ((t_medium != undefined && t_medium == 'emp') || (t_source != undefined && (excludeSources.indexOf(t_source) != -1)));
    return !exclude_medianet_ads;
}

function displayAdknowledgePixel()
{
    var t_medium = getUrlParameter('utm_medium');
    var t_source = getUrlParameter('utm_source');
    return ((t_medium == undefined || t_medium == 'tracking') && (t_source != undefined && t_source == 'e_search'));
}

function renderUndertoneAdUnit() 
{
    var t_medium = getUrlParameter('utm_medium');
    var t_source = getUrlParameter('utm_source');
    
    var excludeSources = ['e_search', 'bl', 'mfp', 'cp', 'pdamv.bidsystem.com', 'kcmv.bidsystem.com'];
    var excludeMediums = ['emp', 'cpc'];

    var exclude_undertone_ads = ((t_medium != undefined && (excludeMediums.indexOf(t_medium) != -1)) ||
                                 (t_source != undefined && (excludeSources.indexOf(t_source) != -1)));
    return !exclude_undertone_ads;
}

function renderYahooNativeAds()
{
    var t_source = getUrlParameter('utm_source');
    
    var excludeSources = ['e_search', 'bl'];

    if (t_source != undefined && ( (excludeSources.indexOf(t_source) != -1) || (t_source.indexOf("optimize.clickshieldfilter.com") != -1)) ) {
        return false;
    } else {
        console.log("Rendering yahoo native ads");
        return true;
    }
}

function renderConditionalAds()
{
    var t_medium = getUrlParameter('utm_medium');
    var t_source = getUrlParameter('utm_source');
    
    var excludeSources = ['bl'];
    var excludeMediums = ['cpc'];

    var exclude_ads = ((t_medium != undefined && (excludeMediums.indexOf(t_medium) != -1)) ||
                       (t_source != undefined && (excludeSources.indexOf(t_source) != -1)));
    
    console.log("In renderConditionalAds() - " + exclude_ads);
    return !exclude_ads;
};
/*
 * jQuery Form Plugin
